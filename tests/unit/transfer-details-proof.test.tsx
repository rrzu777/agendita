...
describe('TransferDetails con comprobante', () => {
  it('requireProof=true deshabilita "Ya transferí" hasta subir', () => {
    const html = renderToStaticMarkup(
      <TransferDetails bank={bank as never} amount={1000} currency="CLP" deadlinePhrase={null} declaring={false} onDeclare={() => {}} bookingId="b1" kind="deposit" />,
    )
  ...
  it('requireProof=false: el botón no está bloqueado por falta de comprobante', () => {
    const html = renderToStaticMarkup(
      <TransferDetails bank={{ ...bank, requireProof: false } as never} amount={1000} currency="CLP" deadlinePhrase={null} declaring={false} onDeclare={() => {}} bookingId="b1" kind="deposit" />,
    )
