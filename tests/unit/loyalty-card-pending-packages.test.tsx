...
describe('LoyaltyCard — paquetes por confirmar', () => {
  it('lista una pending sin declarar con "Te falta transferir" y link de retorno', () => {
    ...
    const html = renderToStaticMarkup(
      <LoyaltyCard customerName="Ana" business={{ name: 'Biz', logoUrl: null, category: 'nails' }} data={data} redeemAction={async () => ({ ok: true, data: undefined })} />,
    )
  ...
  it('sin pendientes no renderiza la sección', () => {
    const html = renderToStaticMarkup(
      <LoyaltyCard customerName="Ana" business={{ name: 'Biz', logoUrl: null, category: 'nails' }} data={baseData} redeemAction={async () => ({ ok: true, data: undefined })} />,
    )
