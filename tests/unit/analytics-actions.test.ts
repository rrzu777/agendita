...
const db = vi.hoisted(() => ({ acquisitionLink: { create: vi.fn(), updateMany: vi.fn() }, promotion: { findFirst: vi.fn() }, analyticsCollectionPeriod: { findFirst: vi.fn(), create: vi.fn(), update: vi.fn(), updateMany: vi.fn() }, $queryRaw: vi.fn(), $executeRaw: vi.fn(), $transaction: vi.fn() }))
...
describe('owner/admin acquisition and collection mutations', () => {
  beforeEach(() => {
    ...
    db.analyticsCollectionPeriod.update.mockResolvedValue({ id: 'period' })
  ...
  it('rotates an open v1 period before opening the configured v2 source contract', async () => {
    vi.stubEnv('OWNER_ANALYTICS_CAPTURE_CONSENT_VERSION', '2')
    db.analyticsCollectionPeriod.findFirst.mockResolvedValue({ id: 'period-v1', consentVersion: 1 })
    expect((await setAnalyticsCollectionEnabled(true)).ok).toBe(true)
    expect(db.analyticsCollectionPeriod.update).toHaveBeenCalledWith({ where: { id: 'period-v1' }, data: { endedAt: expect.any(Date), closeReason: 'version_change' } })
    expect(db.analyticsCollectionPeriod.create).toHaveBeenCalledWith({ data: expect.objectContaining({ consentVersion: 2, definitionVersion: 1 }) })
  })
})
