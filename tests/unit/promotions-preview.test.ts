...
vi.mock('@/lib/auth/server', () => ({ requireBusiness: vi.fn(), requireBusinessRole: vi.fn(), ForbiddenError }))
...
describe('previewPromotion', () => {
  ...
  it('returns discount for a valid code', async () => {
    ...
    expect(res.ok).toBe(true)
    expect(res.ok && res.data).toMatchObject({ ok: true, discount: 4000, finalAmount: 16000, promotionId: 'p1' })
  })
