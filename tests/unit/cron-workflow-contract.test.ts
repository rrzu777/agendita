...
describe('cron workflow contract', () => {
  it('runs every hourly cron through the strict JSON helper', () => {
    ...
    expect(helperSteps).toHaveLength(5)
    expect(helperSteps.every((step) => step['continue-on-error'] === true)).toBe(true)
    expect(new Set(helperSteps.map((step) => step.id)).size).toBe(5)
    ...
    expect(aggregateStep?.env?.SUBSCRIPTION_BILLING_OUTCOME).toBe('${{ steps.subscription_billing.outcome }}')
  ...
  it('invoca los cinco endpoints aunque el primero falle y falla al agregar resultados', async () => {
    const workflow = loadWorkflow('cron.yml')
    ...
    const result = spawnSync('bash', ['-c', aggregateStep.run!], {
      ...
      env: {
        ...
        SUBSCRIPTION_BILLING_OUTCOME: outcomes.subscription_billing,
      },
    })
