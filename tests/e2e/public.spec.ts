...
test.describe('main beta flows', () => {
  ...
  test('owner creates a service and changes weekly availability', async ({ page }) => {
    ...
    await expect(page.getByText('Guardado ✓', { exact: true })).toBeVisible({ timeout: 10000 })
    await page.reload()
    await expect(page.getByRole('heading', { name: 'Horario semanal' })).toBeVisible()
    const visibleMondayStart = page.getByLabel('Lunes inicio', { exact: true }).filter({ visible: true })
    await expect(visibleMondayStart).toHaveCount(1)
    await expect(visibleMondayStart).toContainText('10:00')
  })
})
