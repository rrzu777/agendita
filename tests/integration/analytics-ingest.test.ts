...
describe('real PostgreSQL bootstrap and ingest serialization', () => {
  ...
  it('keeps consent v2 source rows isolated while allowing in-flight v1 tokens to finish their window', async () => {
    const oldInput = sessionInput()
    const oldSession = await bootstrapAnalyticsSession(context, oldInput, captureNow)
    const oldAttemptInput = { bootstrapKey: randomUUID(), credential: oldSession.credential, entryKind: 'complete' as const }
    const oldAttempt = await bootstrapAnalyticsAttempt(context, oldAttemptInput, captureNow)
    vi.stubEnv('OWNER_ANALYTICS_CAPTURE_CONSENT_VERSION', '2')
    const afterRotation = new Date(captureNow.getTime() + 1000)
    await prisma.analyticsCollectionPeriod.updateMany({ where: { businessId, endedAt: null }, data: { endedAt: afterRotation, closeReason: 'version_change' } })
    await prisma.analyticsCollectionPeriod.create({ data: { businessId, definitionVersion: 1, consentVersion: 2, businessTimeZone: context.timezone, startedAt: afterRotation } })
    expect(await bootstrapAnalyticsSession(context, oldInput, afterRotation)).toMatchObject({ id: oldSession.id })
    expect(await bootstrapAnalyticsAttempt(context, oldAttemptInput, afterRotation)).toMatchObject({ id: oldAttempt.id })
    expect((await ingestAnalyticsBatch(context, { credential: oldAttempt.credential, events: [event(1)] }, afterRotation)).receipts[0].status).toBe('accepted')
    expect(await prisma.analyticsSession.findUniqueOrThrow({ where: { id: oldSession.id } })).toMatchObject({ consentVersion: 1 })
    const input = { bootstrapKey: randomUUID(), consent: true as const, consentVersion: 2 as const }
    const receipt = await bootstrapAnalyticsSession(context, input, afterRotation)
    const stored = await prisma.analyticsSession.findUniqueOrThrow({ where: { id: receipt.id } })
    expect(stored.consentVersion).toBe(2)
    expect(verifyAnalyticsCredential(receipt.credential, { businessId, origin: context.origin, secret: captureSecret, now: afterRotation })?.consentVersion).toBe(2)
    await expect(bootstrapAnalyticsSession(context, { ...input, bootstrapKey: randomUUID(), consentVersion: 1 }, afterRotation)).rejects.toMatchObject({ category: 'invalid_request' })
    const attempt = await bootstrapAnalyticsAttempt(context, { bootstrapKey: randomUUID(), credential: receipt.credential, entryKind: 'complete' }, afterRotation)
    expect((await ingestAnalyticsBatch(context, { credential: attempt.credential, events: [event(1)] }, afterRotation)).receipts[0].status).toBe('accepted')
