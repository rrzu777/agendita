...
import { selectWeeklyInsightCandidates } from './selector'
...
export async function runWeeklyInsightsJob(input: { now: Date; cursor: string | null }): Promise<{ processed: number; nextCursor: string | null }> {
  ...
  for (const candidate of selected.candidates) {
    ...
    const insight = await prisma.analyticsWeeklyInsight.upsert({
      ...
      update: { sourceConsentVersion: facts.sourceConsentVersion, status: existing?.status === 'ready' && existing.inputHash === facts.inputHash ? 'ready' : facts.status, facts: facts.facts ?? { version: 1, reasonCode: facts.reasonCode ?? 'insufficient_data' }, inputHash: facts.inputHash, sourceExpiresAt: facts.sourceExpiry, retentionExpiresAt: new Date(Math.min(facts.sourceExpiry.getTime(), candidate.sourceExpiresAt.getTime())), reasonCode: facts.reasonCode ?? null, ...(existing?.status === 'ready' && existing.inputHash === facts.inputHash ? {} : facts.status === 'deterministic_ready' ? { generationStatus: 'not_requested', narrative: Prisma.JsonNull, narrativeGeneratedAt: null } : { generationStatus: 'not_requested' }) },
      select: { id: true },
    })
    if (facts.status === 'deterministic_ready' && facts.facts && performance.now() + MIN_REMAINING_MS < deadline) {
      ...
      if (preference?.aiNarrativeEnabled && preference.privacyVersion === 2) {
        ...
        if (claim) await finalizeWeeklyGeneration({ claim, result: await narrateWeeklyFacts({ facts: claim.facts, model: config.model, now: input.now }), now: input.now })
      }
  ...
  return { processed, nextCursor: processed < selected.candidates.length ? input.cursor : selected.nextCursor }
}
