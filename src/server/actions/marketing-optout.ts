...
/** Tail común de las dos vías públicas: rate limit (mismo bucket keyed por
 *  customer.id — alternar superficies no duplica el cupo), update y revalidate. */
...
export async function setMarketingOptOutAsMe(customerId: string, optedOut: boolean) {
  ...
  await applyOptOut(customer, optedOut, [
    `/mi/${customer.business.slug}`,
    ...(customer.loyaltyToken ? [`/tarjeta/${customer.loyaltyToken}`] : []),
  ])
}
