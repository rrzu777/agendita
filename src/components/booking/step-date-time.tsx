...
export function StepDateTime({ businessId, timezone, data, onDate, onSelect, onBack }: {
  ...
  return <div>
    ...
    <div className="mb-4 flex items-center justify-between gap-2">
      <Button variant="outline" aria-label="Mes anterior" disabled={month <= today.slice(0, 7)} onClick={() => setMonth(format(addMonths(monthDate, -1), 'yyyy-MM'))}>‹</Button>
      <h3 className="font-semibold capitalize">{format(monthDate, 'MMMM yyyy', { locale: es })}</h3>
      <Button variant="outline" aria-label="Mes siguiente" disabled={!preview || nextMonth > preview.windowEnd.slice(0, 7)} onClick={() => setMonth(nextMonth)}>›</Button>
    </div>
    {!current && <p role="status" className="mb-4 text-sm">Consultando días con horas disponibles…</p>}
    {current?.error && <div role="alert" className="mb-4"><p>{current.error}</p><Button variant="outline" className="mt-2" onClick={() => { setResponse(null); setRetry(n => n + 1) }}>Reintentar</Button></div>}
