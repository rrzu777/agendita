...
export function BookingWizard({ businessId, slug, business, timezone, currency, services, professionals, professionalWords, cancellationPolicy, selfServiceCutoffHours, manualHoldHours, referralToken, session }: BookingWizardProps) {
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <section className="rounded-[2rem] border border-border/50 bg-card p-5 shadow-[var(--cream-shadow)] sm:p-8">
        ...
        {currentStep === 'payment' && data.serviceId && data.timeSlot && (
          <StepPayment data={data} updateData={updateData} businessId={businessId} timezone={timezone} currency={currency} cancellationPolicy={cancellationPolicy} selfServiceCutoffHours={selfServiceCutoffHours} manualHoldHours={manualHoldHours} referralToken={referralToken} onSuccess={(creada) => {
            setReserva(creada)
            nextStep()
          }} onBack={prevStep} />
        )}
