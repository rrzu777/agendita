...
export function ClientRouteError({
  ...
  const accountRecovery = area === 'account'
  return (
    <div data-client-route-shell="" data-client-route-area={area} className="min-h-screen bg-background text-foreground">
      ...
      <section className="w-full max-w-md rounded-2xl border border-border bg-card p-6 text-center shadow-sm" aria-labelledby="client-error-title">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">Intenta nuevamente. Si el problema continúa, {accountRecovery ? 'vuelve a tu cuenta y retoma desde allí.' : 'vuelve al inicio y retoma desde allí.'}</p>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Button type="button" size="touch" onClick={retry}>Intentar de nuevo</Button>
          <Button asChild type="button" size="touch" variant="outline"><Link href={accountRecovery ? '/mi' : '/'}>{accountRecovery ? 'Volver a mi cuenta' : 'Volver al inicio'}</Link></Button>
        </div>
      </section>
      </main>
    </div>
