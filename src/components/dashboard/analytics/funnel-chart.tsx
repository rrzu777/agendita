...
export function FunnelChart({ funnel, complete, unavailable }: { funnel: OwnerAnalyticsReport['funnel']; complete: OwnerAnalyticsReport['complete']; unavailable: boolean }) {
  ...
  if (observed.length === 0) return <p className="rounded-lg bg-secondary/50 p-4 text-sm text-muted-foreground">Todavía no hay pasos observados para este período.</p>
  const max = Math.max(...observed.map((step) => step.count), 1)

  return (
    <div className="space-y-3">
      <ol className="space-y-2" aria-label="Recorrido observado">
        {observed.map((step, index) => (
          <li key={step.milestone} className="grid grid-cols-[1.5rem_minmax(6rem,1fr)_3rem] items-center gap-3 text-sm">
            ...
            <span className="min-w-0">
              <span className="block font-medium text-primary">{labels[step.milestone] ?? step.milestone}</span>
      ...
      <p className="rounded-lg border border-border bg-secondary/25 p-3 text-sm text-muted-foreground"><span className="font-semibold text-primary">Recorrido incompleto:</span> {complete.pathIncomplete} conversion{complete.pathIncomplete === 1 ? '' : 'es'} con reserva válida sin una secuencia completa registrada. Se mantienen en la conversión general.</p>
    </div>
  )
}
