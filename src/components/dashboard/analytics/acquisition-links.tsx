    ...
    <section className="space-y-4" aria-labelledby="acquisition-links-title">
      ...
      <div tabIndex={0} aria-label="Desplazar enlaces de adquisición horizontalmente" className="overflow-x-auto rounded-xl ring-1 ring-border/60 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
        <table aria-label="Enlaces de adquisición" className="w-full min-w-[44rem] caption-bottom text-sm">
          ...
          <tbody>{links.rows.length === 0 ? <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">No hay enlaces creados todavía.</td></tr> : links.rows.map((link) => <tr key={link.id} className="border-b hover:bg-muted/50"><td className="p-2 font-medium text-primary">{link.campaignName}<span className="block text-xs font-normal text-muted-foreground">{link.promotionId ? `Promoción asociada: ${promotionLabels[link.promotionId] ?? `nombre no disponible · ${link.promotionId}`}` : 'Sin promoción asociada'}</span></td><td className="p-2">{channels.find(([value]) => value === link.channel)?.[1] ?? link.channel}</td><td className="max-w-[18rem] truncate p-2 font-mono text-xs" title={link.url}>{link.url}</td><td className="p-2">{link.archivedAt ? 'Archivado' : 'Activo'}</td><td className="space-x-1 p-2 text-right"><Button type="button" size="icon-sm" variant="outline" aria-label={`Copiar ${link.campaignName}`} onClick={() => void copy(link.url)}><Copy /></Button>{!link.archivedAt && <Button type="button" size="icon-sm" variant="outline" aria-label={`Archivar ${link.campaignName}`} disabled={pending} onClick={() => archive(link.id)}><Archive /></Button>}</td></tr>)}</tbody>
        </table>
      </div>
