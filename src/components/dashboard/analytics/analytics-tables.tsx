...
const coverageLabels: Record<string, string> = { complete: 'Completa', partial: 'Parcial', disabled: 'Deshabilitada', unknown: 'Desconocida' }
const stateLabels: Record<string, string> = { closed: 'Cerrada', provisional: 'Provisional', frozen: 'Congelada', failed: 'Fallida', unavailable: 'No disponible' }
...
export function AnalyticsTables({ report, pagination }: { report: OwnerAnalyticsReport; pagination: AnalyticsPagination }) {
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <section className="min-w-0 space-y-3">
        ...
        <div tabIndex={0} onKeyDown={scrollWithArrowKeys} aria-label="Desplazar tendencia diaria horizontalmente" className="overflow-x-auto rounded-xl ring-1 ring-border/60 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
          <table aria-label="Tendencia diaria" className={tableClassName}>
            <thead><tr><th className={headClassName}>Fecha local</th><th className={headClassName}>Visitas</th><th className={headClassName}>Intentos completos</th><th className={headClassName}>Conversión completa</th><th className={headClassName}>Reservas creadas completas</th><th className={headClassName}>Intentos parciales</th><th className={headClassName}>Conversión parcial</th><th className={headClassName}>Reservas creadas parciales</th></tr></thead>
            <tbody>{report.trend.map((day) => <tr key={`${day.date}-${day.timezone}`} className="hover:bg-muted/50"><td className={cellClassName}>{day.date} <span className="text-xs text-muted-foreground">({day.timezone})</span></td><td className={`${cellClassName} font-mono tabular-nums`}>{day.visits}</td><td className={`${cellClassName} font-mono tabular-nums`}>{day.complete.attempts}</td><td className={cellClassName}>{rate(day.complete.conversion)}</td><td className={cellClassName}>{day.complete.bookingsCreated}</td><td className={`${cellClassName} font-mono tabular-nums`}>{day.partial.attempts}</td><td className={cellClassName}>{rate(day.partial.conversion)}</td><td className={cellClassName}>{day.partial.bookingsCreated}</td></tr>)}</tbody>
          </table>
        </div>
      ...
      <section className="min-w-0 space-y-3">
        <div><h2 className="font-heading text-xl font-semibold text-primary">Servicios observados</h2><p className="text-sm text-muted-foreground">Grano independiente: no cruza canales ni enlaces. Los montos completados son el valor final de la reserva, no prueba de pago cobrado.</p></div>
        <div tabIndex={0} onKeyDown={scrollWithArrowKeys} aria-label="Desplazar servicios observados horizontalmente" className="overflow-x-auto rounded-xl ring-1 ring-border/60 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
          <table aria-label="Servicios observados" className={tableClassName}>
            <thead><tr><th className={headClassName}>Servicio</th><th className={headClassName}>Interés</th><th className={headClassName}>Seleccionados</th><th className={headClassName}>Quitaron</th><th className={headClassName}>Incompatible</th><th className={headClassName}>Conversión</th><th className={headClassName}>Conversiones sin interés observado</th><th className={headClassName}>Atendidas al consultar</th><th className={headClassName}>Valor completado</th></tr></thead>
            <tbody>{report.services.rows.map((service) => { const economics = report.currentBookings.completedEconomics.status === 'available' ? report.currentBookings.completedEconomics.services.find(row => row.serviceId === service.id) : null; return <tr key={`${service.population}-${service.id}`} className="hover:bg-muted/50"><td className={`${cellClassName} font-medium text-primary`}><span className="block">{service.label}</span><span className="block text-xs font-normal text-muted-foreground">{populationLabel(service.population)}</span></td><td className={`${cellClassName} font-mono tabular-nums`}>{service.interest}</td><td className={`${cellClassName} font-mono tabular-nums`}>{service.selected}</td><td className={`${cellClassName} font-mono tabular-nums`}>{service.removals}</td><td className={`${cellClassName} font-mono tabular-nums`}>{service.incompatible}</td><td className={cellClassName}>{rate(service.conversion)}</td><td className={`${cellClassName} font-mono tabular-nums`}>{service.unobservedConversions}</td><td className={cellClassName}>{economics?.bookings ?? (report.currentBookings.completedEconomics.status === 'available' ? 0 : 'Límite excedido')}</td><td className={cellClassName}>{economics ? new Intl.NumberFormat('es-CL', { style: 'currency', currency: report.currentBookings.currency, maximumFractionDigits: 0 }).format(economics.amount) : report.currentBookings.completedEconomics.status === 'available' ? '0' : 'Límite excedido'}</td></tr> })}</tbody>
          </table>
        </div>
      </section>
      <section className="min-w-0 space-y-3 xl:col-span-2"><div><h2 className="font-heading text-xl font-semibold text-primary">Cobertura por cohorte</h2><p className="text-sm text-muted-foreground">Fecha y zona conservan su identidad; una cohorte no disponible interrumpe la tendencia.</p></div><div tabIndex={0} onKeyDown={scrollWithArrowKeys} aria-label="Desplazar cobertura por cohorte horizontalmente" className="overflow-x-auto rounded-xl ring-1 ring-border/60"><table aria-label="Cobertura por cohorte" className={tableClassName}><thead><tr><th className={headClassName}>Fecha local</th><th className={headClassName}>Zona</th><th className={headClassName}>Cobertura</th><th className={headClassName}>Estado</th></tr></thead><tbody>{report.coverage.cohorts.map(c => <tr key={`${c.date}-${c.timezone}-${c.version}`}><td className={cellClassName}>{c.date}</td><td className={cellClassName}>{c.timezone}</td><td className={cellClassName}>{coverageLabels[c.coverage] ?? 'Desconocida'}</td><td className={cellClassName}>{stateLabels[c.state] ?? 'Desconocido'}</td></tr>)}</tbody></table></div></section>
    </div>
  )
}
