...
function readAlertEmails(strict: boolean): string[] {
  ...
  if (emails.some((email) => email.length > 254 || !emailPattern.test(email))) {
    if (strict) throw new Error('Invalid OWNER_ANALYTICS_ALERT_EMAILS')
    return []
  }
  return emails
}
...
export function getOwnerAnalyticsOperationsConfig(): OwnerAnalyticsOperationsConfig {
  ...
  const alertEmails = readAlertEmails(alertsEnabled)
