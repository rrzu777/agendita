...
export async function resolveLoyaltyCustomer(db: Db, token: string) {
  ...
  const customer = await db.customer.findUnique({
    ...
    select: {
      id: true, name: true, businessId: true, referralToken: true, userId: true, marketingOptOutAt: true,
      business: { select: { id: true, name: true, slug: true, subdomain: true, logoUrl: true, category: true, brandColor: true, visualStyle: true, loyaltyConfig: true } },
    },
  })
