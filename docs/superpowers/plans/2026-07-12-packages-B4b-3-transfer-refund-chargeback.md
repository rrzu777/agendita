...
- **`refundPayment` en la interfaz rompe 5 objetos:** `manual-provider`, `mock-provider`, `createMercadoPagoProvider` (retornado), el wrapper literal `mercadoPagoPaymentProvider` **y** `fakeProvider()` en `src/lib/payments/create-preference.test.ts:9` (implementa `PaymentProvider` inline; está bajo `src/` → rompe el gate `tsc | grep '^src/'` y el `build` de CI, pero vitest NO lo detecta). El case `webpay` de `factory.ts` sólo `throw` (sin objeto). Agregar `refundPayment: vi.fn().mockResolvedValue({ refundId: null, status: 'refunded', rawResponse: {} })` al fake.
...
- [ ] **Step 3b: Actualizar el test existente que rompe** — `src/server/actions/packages.customer.test.ts:20` afirma `where.status === { in: ['active','refunded'] }` (test "excluye compras pending del panel de la dueña"). Con el cambio a `{ in: ['active','refunded','pending','expired'] }` ese test falla en CI. Actualizar la aserción y el título del test en ESTE commit.
...
También agregar `refundPayment` al `fakeProvider()` inline de `src/lib/payments/create-preference.test.ts:9` (5º implementador de la interfaz; rompe `tsc`/`build` si falta): `refundPayment: vi.fn().mockResolvedValue({ refundId: null, status: 'refunded', rawResponse: {} }),`
...
- [ ] **Step 1: Usar la ventana de hold correcta = `holdHours` de `getBankTransferInfo`** (NO `verifyHours`).

**Verificado:** la ventana de la CLIENTA para transferir es `bankTransferAccount.holdHours` (schema `holdHours Int @default(24)`, non-null), como en la creación de reservas por transferencia (`bookings.ts:335`: `bankTransferAccount.holdHours * 60`). NO usar `verifyHours` (`bank-transfer-public.ts:181` — esa es la ventana de la DUEÑA para verificar, nullable). `holdHours` viene expuesto en `getBankTransferInfo(businessId)` (`src/lib/bank-transfer/public-info.ts`, `BankTransferPublicInfo`). El hold del paquete por transferencia = `addHours(now, transferInfo.holdHours)`.
...
**Nota:** el nombre del índice compuesto de Prisma está verificado = `packagePurchaseId_provider_providerPaymentId` (orden exacto del `@@unique`, sin `@@map`).

**Parametrizar el método en `createPackagePurchase`** (hoy `HOLD_MINUTES=30` hardcodeado, sin campo `method` en el input ni en `createPurchaseSchema`). Cambios concretos:
- Agregar `method: 'mp' | 'transfer'` al input de `createPackagePurchase` y a `createPurchaseSchema` (`z.enum(['mp','transfer'])`).
- Cargar `getBankTransferInfo(product.businessId)` dentro de la función SÓLO cuando `method === 'transfer'`; si es `null` (transferencia deshabilitada) → error. El hold = `method === 'transfer' ? addHours(now, info.holdHours) : addMinutes(now, HOLD_MINUTES)`.
- **Reuse de compra pending (`packages-checkout.ts:88-98`) DEBE filtrar por método** — hoy sólo filtra `status:'pending', holdExpiresAt:{gte:now}`. Sin esto, una clienta que arrancó por MP (hold 30 min) y reintenta por transferencia reusaría la compra con el hold equivocado. Agregar al `where` del `findFirst` un discriminador de método. **Decisión:** discriminar por `source` no alcanza (ambos son `'online'`); en su lugar, recalcular el `holdExpiresAt` de la compra reusada al método actual (un `update` del hold en el reuse) — así el reintento por transferencia extiende el hold correcto sin duplicar la compra.
...
- [ ] **Step 3: Introducir una máquina de estados en `package-checkout.tsx`** (hoy es un form PLANO de un paso — card única nombre/teléfono/email/términos → `handleBuy` que llama `createPackagePurchase` e **inmediatamente** `initiatePackagePayment` + redirect, `package-checkout.tsx:28-61`; NO hay `StepPayment` que reusar acá). Trabajo nuevo:
  - Agregar `step: 'form' | 'method' | 'transfer'` (useState).
  - Si sólo hay MP disponible (no `transferInfo`): comportamiento actual intacto (form → `createPackagePurchase({method:'mp'})` → `initiatePackagePayment` → redirect).
  - Si hay transferencia: form → `step:'method'` (elegir MP o transferencia). MP → `createPackagePurchase({method:'mp'})` + `initiatePackagePayment` (redirect). Transferencia → `createPackagePurchase({method:'transfer'})` → `step:'transfer'`: muestra `transferInfo` (datos bancarios) + botón "Ya transferí" → `declarePackageTransfer({ purchaseId })` → navega a `/paquetes/confirmation?purchaseId=`.
  - Prop nueva: `transferInfo: BankTransferPublicInfo | null`.

- [ ] **Step 4: Threadear `transferInfo` por la cadena real de props** — `page → PackagesBusinessPage → PackageCatalog → PackageCheckout` (la page NO renderiza `PackageCatalog` directo; renderiza `PackagesBusinessPage`, `packages-business-page.tsx:16`). `src/app/paquetes/[slug]/page.tsx` y `src/app/paquetes/page.tsx` (ambas `force-dynamic`, ya con `Promise.all`) agregan `getBankTransferInfo(business.id)` al `Promise.all` y lo pasan a `PackagesBusinessPage` → `PackageCatalog` → `PackageCheckout`.
...
- [ ] **Step 4: Copy en `confirmation/page.tsx`** — ⚠️ el `config` (`page.tsx:56-79`) hoy tiene SÓLO las claves `active|pending|rejected` y luego hace `config.icon` (`:79`). Si `derivePackageConfirmationState` devuelve `expired|refunded|disputed` y no existe la clave → `config` es `undefined` → **crash en runtime**. Las 6 claves DEBEN existir en `config`. Agregar `expired` ("Tu compra expiró. Podés iniciarla de nuevo."), `refunded` ("Este pago fue reembolsado."), `disputed` ("Este pago fue reembolsado.") — cada una con su `icon`/`title`/`message`. Distintos de `rejected` ("Pago no aprobado. Podés intentar de nuevo."). Pasar `chargebackAt` al derive (el `findUnique` con include ya trae el escalar post-migración).
...
- [ ] **Step 4: Componente `PendingPackageTransfers`** — lista cada compra con "Confirmar"/"Rechazar" que llaman `confirmPackageTransfer(paymentId)`/`rejectPackageTransfer(paymentId)` (el `paymentId` del pago `bt-pkg-declared` de la compra). Renderizarlo arriba de `/dashboard/paquetes`. **Contador en el home:** el de reservas (`dashboard/page.tsx:59`) se deriva de `bookings.filter(...)` ya cargados — las transferencias de paquete son otra entidad (`PackagePurchase`), NO están en `bookings`, así que hace falta una **query separada** `getPendingPackageTransfers()` y un **contador/banner propio** (no sumar al `PendingTransfersBanner` de reservas). Es la fuente de respaldo del email — el canal real con Resend caído.
...
- [ ] **Step 3: Extender el tipo `db` y agregar el sweep de paquetes.**

⚠️ **Early-return:** `expireStaleHolds` retorna en `:40-42` (`if (expiredBookings.length === 0) return {...}`) — el caso más común (sin reservas vencidas). Si el sweep de paquetes va sólo antes del return final (`:139`), **nunca corre** cuando no hay reservas. Fix: el sweep de paquetes debe correr **incondicionalmente** — extraerlo a una sección propia que corra ANTES del early-return `:40`, o eliminar ese early-return y agregar `packagesExpired: 0` a su objeto de retorno. Además `src/app/api/cron/expire-holds/route.ts:24` sólo revalida `result.businessIds` (negocios de reservas) — agregar los `businessId` de paquetes expirados al set de revalidación (nuevo campo `packageBusinessIds` en el result, o unirlos a `businessIds`).
...
El bloque de paquetes (corre siempre, independiente del sweep de reservas):
