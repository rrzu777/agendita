...
| Rutas | Evidencia automatizada | Navegador local real |
|---|---|---|
| 28 Perfil público | Tema tenant en página/layout y boundary; identidad sólo desde logo/foto/bio/contacto/ubicación/modalidad disponibles; sin señal inventada de verificación; servicios y horarios con vacíos honestos; reseñas ausentes no generan una sección vacía. | `/b/mimosnails` (`balanced`) y `/b/jacke-vas-gutierrez` (`soft`) en superficies representativas de 390×844 y 1440×1000; H1 y título tenant-aware, contenido real, CTA, horarios y 0 overflow horizontal. |
| 29–30 Reserva canónica/alias | Progreso conceptual fijo de seis etapas sin cambiar la máquina dinámica; multi-servicio; profesional opcional y query param; preview combinado; draft/login Google; validación app-owned con required/optional, dirección condicional y `Fecha de nacimiento`; métodos de pago/políticas sólo cuando realmente aplican. | `/book` tenant en 390×844, 834×1112 y 1440×1000, con temas `balanced` y `soft`; se seleccionaron dos servicios, un día y una hora, se llegó a Tus datos y se comprobó foco/error requerido. Se observaron seis etapas y 0 overflow. No se pulsó Google ni se envió la reserva. |
| 31 Confirmación | Tema real del negocio y estados confirmed/verifying/rejected/pending/paid-unconfirmed/verifying-transfer/expired/cancelled, saldo/comprobante, CTA de cuenta y advertencia de cancelación cubiertos por pruebas de componentes/dominio existentes. | No se fabricó una reserva para llegar a confirmación. La ruta se validó mediante build y pruebas automatizadas; sus estados terminales no se recorrieron en navegador local. |

Evidencia fresca final: 27 archivos y 196 tests enfocados verdes en una ejecución, `npm run lint`, `npm run typecheck`, `npm run build` y `git diff --check` exitosos. El detector final de Impeccable devolvió `[]`. La revisión visual no mostró errores de consola de aplicación; sólo mensajes informativos de React/HMR. El alias que redirige a subdominio no conserva el puerto no estándar `3417` en desarrollo local, por lo que el QA tenant continuó con la URL local explícita con puerto; no se cambió el contrato de URL de producción por esa limitación del entorno.
