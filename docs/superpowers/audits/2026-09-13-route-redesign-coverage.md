...
| Rutas | Evidencia automatizada | Navegador local real |
|---|---|---|
| 11–17 Crecimiento | Navegación derivada de la matriz canónica y filtrada por rol; jerarquía de oportunidades antes de metodología; campaña detalle; validación client-side de constraint errors con asociación y foco en promociones, campañas, fidelización, reglas y paquetes | Lista y detalle en 390×844, 834×1112 y 1440×1000; H1, título, navegación `Crecimiento`, sin overflow ni boundary de error; estados con datos y vacíos representativos. No se enviaron campañas ni mutaciones de catálogo. |
...
| 20–24 Configuración | Alias a perfil; navegación local; required/optional veraz; validación client-side con error asociado/foco, draft y recuperación en banco; switches booleanos válidos en `false` sin semántica required | Alias `/dashboard/settings` → `/dashboard/settings/profile`; cuatro destinos en los tres tamaños; tema soft representativo y ruta de pagos repetida con entorno sandbox explícito |
| 26–27 Admin | Zero-state/lista, conteo de atención y salud de cuenta cubiertos por page tests. Pruebas enfocadas de los controles reales cubren apertura/cancelación/foco, targets de 44 px y fallos/pending representativos de suspensión y configuración; no prueban cada acción administrativa ni proveedor real. | Lista/detalle en los tres tamaños; se corrigió overflow por min-content a 390 px; diálogo de suspensión abierto/cerrado con Escape, reduced motion y foco devuelto al trigger de 44 px, sin ejecutar la acción. La configuración de facturación no se confirmó en navegador. |
