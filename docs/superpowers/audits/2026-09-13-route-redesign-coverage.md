...
| Rutas | Evidencia automatizada | Navegador local real |
|---|---|---|
| 28 Perfil público | Tema y `themeColor` tenant-aware con degradación neutral; identidad sólo desde datos disponibles; sin señal inventada de verificación; vacíos honestos; CTA por servicio conserva adquisición, allowlistea `service` y mantiene multiselección. | `/b/mimosnails` en 320×844, 390×844, 834×1112 y 1440×900: 0 overflow; CTA fija 48 px con safe-area; CTA por servicio 44 px. El alias local con puerto no estándar requirió abrir después el host tenant explícito. |
| 29–30 Reserva canónica/alias | Progreso semántico fijo de seis etapas; transiciones centralizadas con foco/scroll en ambos sentidos; multi-servicio; `service+professional` allowlisted y preseleccionado; la tarjeta de profesional no navega; draft/login Google y pago/políticas conservan contratos. Carga canónica/alias comparte componente y el selector global declara cero/truncación. | Host tenant local en 320×844, 390×844, 834×1112 y 1440×900: 0 overflow. A 320 los días midieron 44×48 px; el foco del panel quedó visible bajo sticky en avance y retroceso. Se eligieron dos servicios; con fixture local de dos profesionales, Ana llegó preseleccionada y marcar Luz mantuvo Paso 2/6 hasta `Continuar`. No se ejecutó OAuth ni proveedor de pago. |
| 31 Confirmación | Render controlado cubre success/confirmed, verifying, rejected, pending, expired, cancelled y retry, siempre con nombre/identidad y Paso 6/6; `themeColor` deriva del negocio y degrada neutral; copy remite al estado observable y correo condicional, sin promesa de WhatsApp. | Se abrió a 390×844 la confirmación pendiente del booking sintético creado por el E2E en la base descartable: nombre tenant, Paso 6/6, CTA/copy veraz y 0 overflow, sin pago externo. Los demás estados se ejercitaron por render controlado, no mutando reservas reales. |

Evidencia fresca de la ronda de corrección: 264 tests enfocados verdes en dos ejecuciones (78 + 186); suite completa con 4236 passed, 1 skipped y un fallo de aislamiento de `localStorage` que pasó 12/12 al repetir su archivo solo; `public.spec.ts` aislado en PostgreSQL local: 20/20; `npm run lint`, `npm run typecheck`, `npm run build` y `git diff --check` exitosos. El detector de Impeccable no se volvió a ejecutar: se conservó su único resultado previo `[]`. La revisión visual no mostró errores de consola de aplicación. El alias que redirige a subdominio no conserva el puerto no estándar `3417` en desarrollo local, por lo que el QA tenant continuó con la URL local explícita con puerto; no se cambió el contrato de URL de producción por esa limitación del entorno.
