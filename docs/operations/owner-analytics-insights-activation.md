...
1. Revisar el diff y ejecutar `npx prisma validate`, `npx prisma generate`, typecheck, lint y la suite focal. Aplicar las migraciones aditivas sólo en una copia restaurable; verificar tablas, índices, CHECK y FKs, incluyendo la procedencia del snapshot Booking.
...
outbox. La captura pública tiene una ruta versionada v1/v2, pero el valor por
defecto y el entorno actual siguen en v1. La migración
`20260905140000_owner_analytics_consent_v2` y
`20260905150000_owner_analytics_booking_consent_snapshot` permiten sólo esas
dos versiones y
...
- Consentimiento fuente: con la bandera por defecto, demostrar que v1 continúa
