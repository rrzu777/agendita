...
Las cuatro migraciones aditivas de operaciones/weekly/consentimiento/snapshot ya están aplicadas y `OWNER_ANALYTICS_MONITOR_EXPECTED=true`; el health-check falla por `warning`, `critical`, `not_initialized` o respuesta inválida. Antes de activar captura, alertas o insights todavía se requiere revisión de privacidad/legal, proveedor, destinatarios, presupuesto, capacidad/piloto y las pruebas sintéticas correspondientes. Ver `owner-analytics-insights-activation.md`.
